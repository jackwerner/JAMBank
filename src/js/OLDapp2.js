App = {
    ...
    $.getJSON('../products.json', function(data) {
      ...
      for (i = 0; i < data.length; i ++) {
      	if (data[i].forSale == "true"){
			...
			prodTemplate.find('button').attr('data-sellerAddress',data[i].sellerAddress);
  ...
  handlePurchase: function(event) {
	...
	var seller = $(event.target).data('selleraddress'); //must be lowercase
	console.log(seller);
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.Purchasing.deployed().then(function(instance) {
        ...
        console.log(buyer);
        console.log(prodId);
        return purchasingInstance.purchase(buyer, seller, weiprice, prodId, prodURI, {from: buyer, value: weiprice}); //maybe do value: weiprice
